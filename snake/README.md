Implementation of Snake
-
This was done as a starting excecise to get used to raylib and learn some good practices, as well as identify some bad ones for future refernce (e.g. the death screen implementation)

TODOS:
-
  - apple image ✅
  - apple eat sound
  - smooth snake movement
  - fps
  - restart ✅

Build Requirements:
-
  - raylib 5.5 (the makefile assumes raylib to be in an include path; otherwise, it must be modified)
  - no idea what else, sorry (for reference, this was done on Ubuntu 24.04 with WSL)

Build Instructions:
-
  - clone repo
  - run the make command
